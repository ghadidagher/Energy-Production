...
given by  
\[
AICc=-2log(L)+2k+\frac{k(k+1)}{T-k-1}
...
![formula](https://render.githubusercontent.com/render/math?math=AICc=-2log\(L\)+2\(p+q+k+1\)+\\frac%7B2\(p+q+k+1\)\(p+q+k+2\)%7D%7BT-p-q-k-2%7D)
\[
AICc=-2log(L)+2(p+q+k+1)+\frac{2(p+q+k+1)(p+q+k+2)}{T-p-q-k-2}
